package com.doomonafireball.masterbuilder.android.widget;
...
/**
 * An ImageView that supports pinch-to-zoom, double-tap-to-zoom, and swipe panning. <p/> Usage: - Set image source via
 * android:src, setImageBitmap(), or setImageResource() - Supports scale type of CENTER_CROP (default) and CENTER_INSIDE
 * - Default maximum zoom scale is 2.5x
 *
...
//                    }
    ...
    public void setIsCenterInside(boolean b) {
        this.isCenterInside = b;
    }
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (event.getPointerCount() == 2) {
                    int pointerRemaining = event.getAction() == MotionEvent.ACTION_POINTER_2_UP ? 0 : 1;
    ...
    private class TapSwipeListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            mode = NONE;
            mFlingTask = new FlingTask(timeOfLastMoveEvent, velocityX / 1000, velocityY / 1000).execute((Void[]) null);
            return true;
        }
    }
            ...
            if (viewWiderThanImg || viewTallerThanImg) {
                if (viewWiderThanImg && !viewTallerThanImg) {
                    ...
                    matrix.postScale(mScaleFactor, mScaleFactor,
                            detector.getFocusX(), viewHeight / 2);
                } else {
                    matrix.postScale(mScaleFactor, mScaleFactor, viewWidth / 2,
                            viewHeight / 2);
                }
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            if (viewWiderThanImg || viewTallerThanImg) {
                if (viewWiderThanImg && !viewTallerThanImg) {
                    ...
                    matrix.postScale(scaleFactor, scaleFactor, centerX,
                            viewHeight / 2);
                } else {
                    matrix.postScale(scaleFactor, scaleFactor, viewWidth / 2,
                            viewHeight / 2);
                }
                if (scaleFactor < 1) {
                    matrix.getValues(m);
    ...
    public boolean canScrollLeft() {
        matrix.getValues(m);
        return m[Matrix.MTRANS_X] < 0;
    }

    public boolean canScrollRight() {
        matrix.getValues(m);
        return -m[Matrix.MTRANS_X] < right;
    }

    public void toggleZoom(float centerX, float centerY, float maxScale) {
        ZoomAnimation anim;
        if (saveScale > 1) {
            anim = new ZoomAnimation(saveScale, 1, centerX, centerY);
        } else {
            anim = new ZoomAnimation(1, maxScale, centerX, centerY);
        }
        startAnimation(anim);
    }
